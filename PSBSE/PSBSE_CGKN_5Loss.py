...
class AutoEncoder(nn.Module):
    def __init__(self, input_size, hidden_size):
        super().__init__()
        self.encoder = nn.Sequential(nn.Linear(input_size, 16), nn.ReLU(),
        ...
        self.decoder = nn.Sequential(nn.Linear(hidden_size, 16), nn.ReLU(),
                                     nn.Linear(16, 32), nn.ReLU(),
                                     nn.Linear(32, 64), nn.ReLU(),
                                     nn.Linear(64, 64), nn.ReLU(),
                                     nn.Linear(64, 32), nn.ReLU(),
...
cgnn = CGNN(dim_u1, dim_z).to(device)
...
for ep in range(1, epochs+1):
    ...
    for u, u_dot in train_loader:
        ...
        # Encoder
        z = sdefunc.autoencoder.encoder(u[:, dim_u1:])
        # Decoder
        u2_ae = sdefunc.autoencoder.decoder(z)
        loss_ae = nnF.mse_loss(u[:, dim_u1:], u2_ae)

        u_extended = torch.cat([u[:, :dim_u1], z], dim=-1)
        # Latent One-Step Future State
        u_extended_dot_pred = sdefunc(None, u_extended)
        u_extended_pred = u_extended + u_extended_dot_pred*dt
        # Decoder
        u2_pred = sdefunc.autoencoder.decoder(u_extended_pred[:, dim_u1:])
        u_pred = torch.cat([u_extended_pred[:, :dim_u1], u2_pred], dim=-1)
        loss_forecast = nnF.mse_loss(u+u_dot*dt, u_pred)

        # Encoder (Future State)
        z = sdefunc.autoencoder.encoder(u[:, dim_u1:]+u_dot[:, dim_u1:]*dt)
    ...
    train_loss_forecast_z_history.append(train_loss_ae)
...
# with torch.no_grad():
#     test_z = sdefunc.autoencoder.encoder(test_u[:, dim_u1:])
# test_u_extended = torch.cat([test_u[:, :dim_u1], test_z], dim=-1)
# with torch.no_grad():
#     test_u_extended_dot_pred = sdefunc(None, test_u_extended)
# test_u_extended_pred = test_u_extended + test_u_extended_dot_pred * dt
# with torch.no_grad():
#     test_u2_pred = sdefunc.autoencoder.decoder(test_u_extended_pred[:, dim_u1:])
...
with torch.no_grad():
    train_z = sdefunc.autoencoder.encoder(train_u[:, dim_u1:])
train_u_extended = torch.cat([train_u[:, :dim_u1], train_z], dim=-1)
...
def CGFilter(sdefunc, sigma, u1, mu0, R0):
    # u1: (t, x, 1)
    # mu0: (x, 1)
    # R0: (x, x)
    device = u1.device
...
# for _ in range(100):
#     print(_)
#     autoencoder = AutoEncoder(dim_u2, dim_z)
#     cgnn = CGNN(dim_u1, dim_z)
#     sdefunc = SDEFunc(autoencoder, cgnn)
#     idx_randn = torch.from_numpy(np.random.choice(Ntrain-10000+1, size=1))
#     u1 = train_u[idx_randn:idx_randn+10000, :dim_u1].unsqueeze(-1)
...
for ep in range(1, epochs+1):
    ...
    # Encoder
    z_short = sdefunc.autoencoder.encoder(u_short[:, :, dim_u1:])
    z0_short = z_short[0]
    u_extended0_short = torch.cat([u_short[0, :, :dim_u1], z0_short], dim=-1)
    # Integration
    ...
    # Decoder
    u2_pred_short = sdefunc.autoencoder.decoder(u_extended_pred_short[:, :, dim_u1:])
    ...
    # autoendocer
    z_long = sdefunc.autoencoder.encoder(u_long[:, dim_u1:])
    u2_ae_long = sdefunc.autoencoder.decoder(z_long)
    loss_ae = nnF.mse_loss(u_long[:, dim_u1:], u2_ae_long)

    # Filter
    mu_z_pred_long = CGFilter(sdefunc, sigma_hat, u_long[:, :dim_u1].unsqueeze(-1), mu0=torch.zeros(dim_z, 1).to(device), R0=0.01*torch.eye(dim_z).to(device))[0].squeeze(-1)
    loss_da_z = nnF.mse_loss(z_long[cut_point:], mu_z_pred_long[cut_point:])
    # Decoder
    mu_pred_long = sdefunc.autoencoder.decoder(mu_z_pred_long[cut_point:])
    loss_da = nnF.mse_loss(u_long[cut_point:, dim_u1:], mu_pred_long)

    loss_total = loss_forecast + loss_da + loss_ae + loss_forecast_z + loss_da_z
    ...
    train_loss_da_z_history.append(loss_da_z.item())
    ...
    print(ep, " time:", round(end_time-start_time, 4),
          ...
          " loss fore z:", round(loss_forecast.item(), 4),
          " loss da z:", round(loss_da_z.item(),4))
...
# Data Assimilation
with torch.no_grad():
    test_mu_z_pred, test_R_z_pred = CGFilter(sdefunc, sigma_hat, test_u[:, :dim_u1].unsqueeze(-1).to(device), torch.zeros(dim_z, 1).to(device), 0.01 * torch.eye(dim_z).to(device))
...
with torch.no_grad():
    xxxx = sdefunc.autoencoder.encoder(test_mu_pred)
nnF.mse_loss(test_mu_z_pred.squeeze(-1), xxxx)
...
for i in range(test_num_batches):
    ...
    with torch.no_grad():
        test_z_batch = sdefunc.autoencoder.encoder(test_u_batch[:, dim_u1:])
    test_u_extended_batch = torch.cat([test_u_batch[:, :dim_u1], test_z_batch], dim=-1)
    with torch.no_grad():
